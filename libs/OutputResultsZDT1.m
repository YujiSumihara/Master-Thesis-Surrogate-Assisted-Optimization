function [] = OutputResults(Parent, JxParent, ExtJxParent, MFEA, Database, Config)
    ...
    if Config.Operation.Problem == 5
        ...
        [rank_s, ~, index_s] = TruncateIndex(FXs, Config.Metaheuristic.Size);
        ParetoSet_s = X(index_s,:);
        ParetoFront_s = FXs(index_s,:);
        ParetoFront_s = ParetoFront_s(rank_s(index_s) == 1,:);
    
        [FX, ~, ~, ExtFX] = CostFunction(X, Database, Config);
    
        [rank_r, ~, index_r] = TruncateIndex(FX, Config.Metaheuristic.Size);
        ParetoSet = X(index_r,:);
        ParetoFront = FX(index_r,:);
        ParetoFront = ParetoFront(rank_r(index_r) == 1,:);
    
        PFtrue = TrueParetoZDT1(500);
    
        figure('units','normalized','outerposition',[0 0 1 1]);
    
        subplot(1,3,1)
        scatter(FXs(:,1), FXs(:,2), 25, 'filled');
        xlabel('f_1'); ylabel('f_2');
        title(sprintf('Model Function - N = %d', size(FXs,1)));
        grid on; xlim([0 1]); ylim([0 8]);
    
        subplot(1,3,2)
        plot(PFtrue(:,1), PFtrue(:,2), 'k-', 'LineWidth', 2); hold on;
        scatter(ParetoFront_s(:,1), ParetoFront_s(:,2), 35, 'filled');
        xlabel('f_1'); ylabel('f_2');
        title(sprintf('Model Pareto Front - N = %d', size(ParetoFront_s,1)));
        grid on; xlim([0 1]); ylim([0 1.2]);
    
        subplot(1,3,3)
        plot(PFtrue(:,1), PFtrue(:,2), 'k-', 'LineWidth', 2); hold on;
        scatter(ParetoFront(:,1), ParetoFront(:,2), 35, 'filled');
        xlabel('f_1'); ylabel('f_2');
