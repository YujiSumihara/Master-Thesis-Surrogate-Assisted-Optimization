...
for runid = 1:nRuns
    ...
    for a = 1:numel(algorithms)
        ...
        Config = run_experiment_2_3_config_only();
        ...
        save(fullfile(matFolder, 'results_Coffee_compare_partial.mat'), ...
...
save(fullfile(matFolder, 'results_Coffee_compare_final.mat'), ...
     'Resultados', 'PF_real_global', 'algorithms', 'metricFrontType');

Summary = BuildSummaryTableMulti(Resultados, algorithms);
disp(Summary);
writetable(Summary, fullfile(tablesFolder, 'summary_MODE_MOPSO_MOEAD.csv'));

%% Summary of computational cost
CostSummary = BuildCostSummaryTable(Resultados, algorithms);
disp(CostSummary);
writetable(CostSummary, fullfile(tablesFolder, 'CostSummaryCoffee_MODE_MOPSO_MOEAD.csv'));
...
if nRuns >= 2
    ...
    disp(Stats);
    writetable(Stats, fullfile(tablesFolder, 'Wilcoxon_MODE_MOPSO_MOEAD.csv'));
else
...
save(fullfile(matFolder, 'Compare_full_MODE_MOPSO_MOEAD.mat'), ...
...
function PlotBestParetoMulti(Results, PF_real_global, algorithms, metricFrontType)
    ...
    legendEntries(1) = 'Pareto Real Global';
    ...
    xlabel('f_1');
    ylabel('f_2');
    title(sprintf('Melhores fronteiras por menor IGD (%s)', metricFrontType));
